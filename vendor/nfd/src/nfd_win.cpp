...
static int AppendExtensionToSpecBuf( const char *ext, char *specBuf, size_t specBufLen )
{
    ...
    if ( strlen(specBuf) > 0 )
    {
        strncat_s( specBuf, specBufLen * sizeof(char), SEP, specBufLen - strlen(specBuf) - 1 );
    ...
    strncat_s( specBuf, specBufLen * sizeof(char), extWildcard, specBufLen - strlen(specBuf) - 1 );
